...
def to_structured_input_array(input_model: DataModel, input_vars: List[str]) -> np.ndarray:
...
def simulate_fmu_with_inputs(
    ...
    structured_input = to_structured_input_array(input_model, input_vars)
...
def build_dash_layout(
    ...
    def make_fig(var_list: List[str], title: str) -> go.Figure:
        present = [v for v in var_list if v in result.outputs]
        if not present:
            fig = go.Figure()
            fig.update_layout(title=f"No {title.lower()} available.")
            return fig
    ...
    return html.Div([
        dcc.Tabs([
            dcc.Tab(label="Inputs", children=[
                dcc.Graph(figure=make_fig(input_vars, "Inputs"))
            ]),
            dcc.Tab(label="Outputs", children=[
                dcc.Graph(figure=make_fig(output_vars, "Outputs"))
            ])
        ])
    ])
...
if __name__ == "__main__":
    ...
    # generate two independent step inputs
    input_data = step_input(
        fmu_path           = fmu_path,
        step_input_name    = ["INPUT_temperature_cold_circuit_inlet", "INPUT_massflow_cold_circuit", "SETPOINT_temperature_lube_oil", "INPUT_engine_load_0_1"],
        start_time         = 0.0,
        dt                 = 1.0,
        stop_time          = 10*60.0,
